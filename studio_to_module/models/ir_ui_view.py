...
class IrUiView(models.Model):
    ...
    def cleanup_converted_views(self):
        ...
        for view in views_to_delete:
            # Double-check that view still exists (might have been deleted by another process)
            if not view.exists():
                _logger.info(
                    "View %s (ID: %s) no longer exists, skipping cleanup",
                    view.name,
                    view.id,
                )
                continue
            ...
            if view.target_module_id and view.target_module_id.state == "installed":
                ...
                try:
                    with self.env.cr.savepoint():
                        ...
                        # Now try to delete the parent Studio view (only if it still exists)
                        if view.exists():
                            view.unlink()
                            _logger.info(
                                "Successfully deleted Studio view %s (ID: %s)",
                                view.name,
                                view.id,
                            )
                        else:
                            _logger.info(
                                "Studio view %s (ID: %s) was already deleted",
                                view.name,
                                view.id,
                            )
                    ...
                    _logger.warning(
                        "Failed to cleanup Studio view %s (ID: %s): %s",
                        view.name,
                        view.id,
                        str(e),
                    )
                    # Mark as not pending cleanup if we can't delete it
                    # Only attempt to write if the view still exists
                    try:
                        with self.env.cr.savepoint():
                            if view.exists():
                                view.write({"pending_cleanup": False})
                                _logger.info(
                                    "Unmarked view %s (ID: %s) from pending cleanup",
                                    view.name,
                                    view.id,
                                )
                    except Exception:
