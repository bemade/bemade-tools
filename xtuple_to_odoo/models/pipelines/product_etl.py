...
class XtupleProductImporter(models.AbstractModel):
    ...
    def transform_products(self, ctx: ETLContext, extracted: Dict) -> Dict:
        ...
        # Transform products that exist in Odoo by default_code and need xTuple
        # fields set.  Use _transform_product so the saleable-vs-purchased
        # name/description_sale mapping and the empty-default_code rule apply
        # on the update path too (AC4 — no legacy override survives).
        # The load step will pop ``_lookup_code`` for the product search and
        # handle the UoM side-effect (raw-SQL uom_id/uom_po_id write).
        update_vals = []
        for product in products_to_update:
            vals = self._transform_product(product)
            # Pass the current default_code (item_number) so load can look the
            # product up; also pass the resolved UoM ID for the side-effect.
            vals["_lookup_code"] = product.get("item_number")
            update_vals.append(vals)
